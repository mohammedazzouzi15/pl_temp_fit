...
def Gen(EX, CT, I0, D):
    if CT.off == 1:
        ...
        Gen_EX = trapz(EX.ka_hw * np.exp(-((D.hw.reshape(-1,1) - I0.Laser_hw) ** 2) / I0.Laser_B**2), axis=0)
        Gen_CT = trapz(CT.ka_hw * np.exp(-((D.hw.reshape(-1,1) - I0.Laser_hw) ** 2) / I0.Laser_B**2), axis=0)
