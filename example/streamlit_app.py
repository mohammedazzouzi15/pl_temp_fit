...
def display_workflow_status():
    ...
    status_items = [
        ...
        ("⚙️ Config Saved", config_saved),
        ("🔄 Fit Completed", fit_completed),
    ]
...
def data_management_tab():
    ...
    st.header("📂 Data Management & Modification")
    ...
    with col_controls:
        ...
        if "exp_data" in st.session_state:
            ...
            with st.expander("🔧 Data Modification", expanded=True):
                ...
                # Modify button
                if st.button("🔄 Apply Modifications", type="secondary"):
                    with st.spinner("Modifying data..."):
                        try:
                            ...
                            st.success(f"✅ Data modified: {xsc_name}")

                        except Exception as e:
                            st.error(f"❌ Error modifying data: {e!s}")
        ...
        if "pl_fig_interactive" in st.session_state:
            ...
            with plot_tab2:
                if (
                    ...
                    st.info(
                        "📊 Apply data modifications to see comparison plot"
                    )
        else:
            st.info("📊 Load data to see interactive visualization")
            st.markdown("""
            **🎨 Interactive Features:**
            - 🔍 **Zoom & Pan**: Click and drag to zoom, double-click to reset
            - 📊 **Hover Details**: Hover over data points for precise values
            - 👁️ **Legend Toggle**: Click legend items to show/hide data series
            - 📥 **Download**: Use the download button for high-resolution images
            """)
...
def data_modification_tab():
    """Legacy function - now redirects to combined data management."""
    st.info(
        "📋 Data loading and modification have been combined into the Data Management tab."
    )
    st.markdown(
        "👈 Please use the **Data Management** tab for all data operations."
    )
...
def model_configuration_tab():
    ...
    with col1:
        ...
        generate_button = st.button(
            "💾 Generate and Save Config", type="primary"
        )
        ...
        if generate_button:
            ...
            # except Exception as e:
            #   st.error(f"❌ Error generating config: {str(e)}")

    with col2:
        st.subheader("📊 Configuration Actions")

        # plot State Diagram
        if st.button("⚡ Plot State Diagram", type="secondary"):
            if "test_id" not in st.session_state:
                st.error("⚠️ Please generate config first")
            else:
                with st.spinner("Generating interactive state diagram..."):
                    try:
                        ...
                        st.error(f"❌ Error plotting state diagram: {e!s}")
        # Plot fit limits
        if st.button("📈 Preview Fit Limits", type="secondary"):
            if "test_id" not in st.session_state:
                st.error("⚠️ Please generate config first")
            else:
                with st.spinner("Generating interactive fit limits plot..."):
                    try:
                        ...
                        st.error(f"❌ Error plotting fit limits: {e!s}")
...
def fitting_tab():
    ...
    if "test_id" not in st.session_state:
        st.warning("⚠️ Please generate and save model configuration first")
        return
    ...
    with col1:
        ...
        st.info(f"📋 Current Config: **{test_id}**")
        ...
        if run_fit_button:
            ...
            with st.spinner(
                ...
                try:
                    ...
                    st.success("📊 Results loaded automatically!")
        ...
        if st.button("📖 Load Existing Results", type="secondary"):
            with st.spinner("Loading results..."):
                try:
                    ...
                    st.error(f"❌ Error loading results: {e!s}")

    with col2:
        st.subheader("📈 Fitting Status")
        ...
        if "reader" in st.session_state:
            ...
            st.success("🎉 Fitting results are available for analysis!")
        else:
            st.info("⏳ No fitting results available yet")
...
def main():
    ...
    # App header
    st.title("🔬 PL Temperature Fitting Workflow")
    st.markdown(
    ...
    with st.sidebar:
        ...
        st.success("✅ Directories configured")
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📂 Data Management", "⚙️ Model Config", "🔄 Fitting", "📊 Analysis"]
    )
    ...
    with tab4:
        ...
        if "reader" not in st.session_state:
            st.warning("⚠️ Please complete fitting and load results first")
        else:
            st.success(
                "✅ Results loaded - Use the interactive plotting interface below"
            )
            st.info(
                "🎨 **Interactive Plots**: All analysis plots are now interactive! You can zoom, pan, hover for details, and download plots."
            )
            plot_selection()
