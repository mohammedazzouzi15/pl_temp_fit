...
class PLAbsAndLifetime(SpectralDataGeneration):
    def __init__(self, temperature_list, hws_list):
        ...
        self.error_in_lifetimes = {temp: self.relative_error_lifetime * lifetime 
    ...
    def log_probability(self, theta, exp_data, inv_co_var_mat_pl):
        ...
        if lp == -np.inf:
            return -np.inf, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None #do not forget to return the correct number of outputs
        ...
        #sum all the errors
        max_abs_pos_error = (data.D.hw[data.D.alpha[:, -1].argmax()] - self.max_abs_pos_exp)**2 / (2 * self.error_in_max_abs_pos**2)
        ...
        for temp, exp_lifetime in self.temperature_lifetimes_exp.items():
            try:
                ...
                calculated_lifetime = 1 / (data.EX.knr[0][index] + data.EX.kr[index])
...
#        log_prob += -(exciton_lifetime - self.lifetime_exp_high_temp)**2 / self.error_in_lifetime_high_temp**2/2
        ...
        if np.isnan(log_like) or np.isinf(log_like) or log_like is None:
            return -np.inf, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None
