...
class PLAbsAndLifetime60K(SpectralDataGeneration):
    ...
    def log_probability(self, theta, exp_data, inv_co_var_mat_pl):
        ...
        if lp == -np.inf:
            return (
                ...
                None,
                None,
            )  # ,None do not forget to return the correct number of outputs
        ...
        log_prob += (
            -(
                (
                    ...
                    - self.max_abs_pos_exp
                )
                ** 2
            )
            / self.error_in_max_abs_pos**2
            / 2
        )
        exciton_lifetime = 1 / (
            data.EX.knr[0][-7] + data.EX.kr[-7]
        )  # iterative here,
        log_prob += (
            -((exciton_lifetime - self.lifetime_exp_high_temp) ** 2)
            / self.error_in_lifetime_high_temp**2
            / 2
        )
