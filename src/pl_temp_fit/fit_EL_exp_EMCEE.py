...
def main(
    ...
    init_params = [
        hws_PL[np.argmax(Exp_data_PL[:, 0])] - 0.1,
        10,
        0.1,
        0.1,
        0.16,
    ...
    co_var_mat_PL, co_var_mat_EL, variance_EL, variance_PL = (
        plot_generated_data(
            ...
            LE_params,
            savefig=True,
            true_parameters=init_params,
        )
    )

    # get maximum likelihood estimate
    soln = get_maximum_likelihood_estimate(
        Exp_data_EL,
        Exp_data_PL,
        co_var_mat_PL,
        co_var_mat_EL,
...
def plot_generated_data(
    ...
    for x in range(numnber_of_samples):
        model_data_PL, model_data_EL, true_parameters = generate_data(
            temperature_list_EL,
            hws_EL,
            temperature_list_PL,
            hws_PL,
        ...
        def add_plot_to_ax(
            model_data, hws, temperature_list, ax, true_model_list
        ):
            data_true_plot = model_data.reshape(len(hws), -1)
            data_true_plot = data_true_plot / max(
                data_true_plot.reshape(-1, 1)
            )
            true_model_list.append(
                data_true_plot.reshape(len(hws), len(temperature_list))
                + np.random.normal(
                    0, 0.01, size=(len(hws), len(temperature_list))
                )
            )
        ...
        true_model_pl_list = add_plot_to_ax(
            model_data_PL,
            hws_PL,
            temperature_list_PL,
            ax[0][0],
        ...
        true_model_el_list = add_plot_to_ax(
            model_data_EL,
            hws_EL,
            temperature_list_EL,
            ax[0][1],
...
def el_trial(
    theta, temperature_list_EL, hws_EL, temperature_list_PL, hws_PL, theta_Ex
):
    ...
    data.D.kEXCT = (
        10**k_EXCT
    )  # * np.exp(-(data.EX.E - data.CT.E)**2 * ((data.c.kb * 300) / 0.01 / (data.c.kb * data.D.T)))
...
def log_probability(
    theta, data_EL, data_PL, co_var_mat_EL, co_var_mat_PL, X, LE_params
):
