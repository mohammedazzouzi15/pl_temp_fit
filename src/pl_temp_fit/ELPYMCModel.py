...
class ELPYMCModel(ModelBuilder):
    ...
    def build_model(self, X, y, sigma =0.1, **kwargs):
        ...
        truemodel_pl = y
        hws=X['hws']
        logl = LogLike(self.el_loglike, truemodel_pl, sigma, temperature_list, hws)
    ...
    def el_loglike(self, theta, data, sigma, temperature_list, hws):
        if type(theta) == np.ndarray:
            theta = theta.tolist()
        data = data/np.max(data.reshape(-1, 1))
        #hws = hws+np.random.normal(0, 0.1, size=(len(hws), 1))
        model_data = el_trial(list(theta), temperature_list, hws)
    ...
    def plot_posterior_prediction(
        ...
        data_plot = el_trial(theta_mean_list, temperature_list, hws)
