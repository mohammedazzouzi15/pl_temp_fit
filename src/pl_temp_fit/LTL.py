...
from pl_temp_fit.PL_Events import Gen, Emi, SUM
from pl_temp_fit.Recombination_Rates import krad, kabs, knonrad
from pl_temp_fit.EL_events import EL_gen
...
class Constants:
    ...
    E0 = 4 * np.pi * 8.85e-12 * 6.242e18 # Vacuum permittivity (eV * Angstroms / (elementary charge)^2)
...
class Data:
    def __init__(self):
        self.EX = State(E=1.4, vmhigh=5, vmlow=15, sigma=0.02, numbrstates=20, off=0, LI=0.1, L0=0.1, H0=0.15, fosc=5)
        self.CT = State(E=1.1, vmhigh=2, vmlow=2, sigma=0.01, numbrstates=20, off=1, LI=0.1, L0=0.1, H0=0.15, fosc=5)
        self.D = DataParams()
        self.I0 = LightSource()
        self.c = Constants()
        self.xParam = {"coeff": 1}
...
class State:
    def __init__(self, E, vmhigh, vmlow, sigma, numbrstates, off, LI=0.1, L0=0.1, H0=0.15, fosc=0.5, dmus=3.0):
        self.E = E  # mean energy of the state
        self.knr = None
        self.kr = None
        self.ka_hw = None
        self.vmhigh = vmhigh  # number of vibrational states above the initial state
        self.vmlow = vmlow  # number of vibrational states below the final state
        self.sigma = sigma
    ...
    def calculate_DG0(self):
        return np.linspace(self.E - 5 * self.sigma, self.E + 5 * self.sigma, self.numbrstates)
        """
        try :
        except:
            import pytensor.tensor as at
            return at.linspace(self.E - 5 * self.sigma, self.E + 5 * self.sigma, self.numbrstates)
        """


class DataParams:
    def __init__(self):
        self.T = np.linspace(90, 300, 5)  # Example value, replace with your actual data
