...
def FCWD_v(State, const, hw, T):
    ...
    temp, hw_mat, dg0_mat, final_mat, initial_mat = np.meshgrid(
        T, hw, State.DG0, w_array, t_array
    )
    ...
    component2 = (
        np.exp(-hrf)
        * hrf ** (final_mat - initial_mat)
        * factorial(initial_mat)
        / factorial(final_mat)
    )
    component3 = lagur**2
    component4 = np.exp(
        -(
            (
                ...
                - dg0_mat
                + State.Lo
                + (final_mat - initial_mat) * State.hO
            )
            ** 2
        )
    ...
    component5 = np.exp(
        (-initial_mat * State.hO) / (np.array([const.kb]) * temp)
    )
    component6 = np.exp(
        (-dg0_mat + energy_offset) / (np.array([const.kb]) * temp)
    )
    FCWD_n = (
        component1
        * component2
...
def tdm(State, const):
    M = np.sqrt(
        ...
        * const.hbar
        * State.fosc
        / (2 * np.pi * (State.E - State.hO) * const.m_e)
    )
    return M
        ...
        for j in range(len(State.DG0)):
            int[j, i] = (
                (
                    xParam["coeff"]
                    * np.exp(
                        -((State.DG0[j] - State.E) ** 2) / (2 * State.sigma**2)
                    )
                    + (1 - xParam["coeff"])
